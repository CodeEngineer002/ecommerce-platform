...
export default async function ProductDetailPage({ params }: Props) {
  ...
  // Non-locale path has no country segment — use USD as the default since
  // the legacy backfill captured every variant.price into the USD row.
  // (Locale-aware /[country]/[lang]/products page uses the customer's currency.)
  const variantIds = product.variants.map((v) => v.id);
  const resolved = variantIds.length > 0
    ? await resolveVariantPrices(createServiceClient(), variantIds, "USD")
    : new Map();
