...
export default function AdminPagesModulePage() {
  ...
  const { data: pages = [], isLoading, refetch } = useLocalizedCmsPages(country, lang);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <PageHeader
          title="CMS Pages"
          description={`${pages.length} page${pages.length !== 1 ? "s" : ""} in ${localeId}`}
        />
      ...
      {pages.length === 0 ? (
        <div className="rounded-lg border border-dashed p-12 text-center">
          <p className="text-muted-foreground">No pages for {localeId} yet.</p>
          <PermissionGate permission={PERMISSIONS.CMS_EDIT}>
          ...
          {pages.map((page) => (
            <Card key={page.id} className="group">
              <CardHeader className="pb-2">
                ...
                <InheritanceBadge
                  scopeType={(page as Record<string, unknown>).scope_type as "country" | "locale" | null}
                  overrideStatus={(page as Record<string, unknown>).override_status as "inherited" | "overridden" | "detached" | null}
                  inheritsFromId={(page as Record<string, unknown>).inherits_from_id as string | null}
                  module="pages"
                  country={country}
                  lang={lang}
                  contentId={page.id}
        ...
        <DialogContent className="max-w-2xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "content" | "seo")}>
              ...
              <TabsContent value="content" className="space-y-4 pt-2">
                ...
                <FormField
                  ...
                  required
                  error={errors.slug}
                  description={`URL: /${country}/${lang}/pages/{slug}`}
                  {...register("slug")}
                />
                <FormField label="Content (HTML / Markdown)" as="textarea" rows={10} {...register("content")} />
                <div className="flex items-center gap-2">
                  <input type="checkbox" id="is_active" className="rounded" {...register("is_active")} />
                  <label htmlFor="is_active" className="text-sm font-medium">
                    <Eye className="mr-1 inline h-3.5 w-3.5" /> Published (visible on storefront)
                  </label>
                </div>
              </TabsContent>
            ...
            <div className="flex justify-end gap-2 border-t pt-4">
              ...
              <PermissionGate permission={PERMISSIONS.CMS_EDIT} fallback={<Button type="submit" disabled>No edit permission</Button>}>
                <Button type="submit" loading={isPending}>{editItem ? "Save Changes" : "Create Page"}</Button>
              </PermissionGate>
            </div>
          </form>
        </DialogContent>
