    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <PageHeader
          ...
          description={`Reusable locale-scoped blocks for ${localeId}`}
        />
      ...
      {blocks.length === 0 ? (
        <div className="rounded-lg border border-dashed p-12 text-center">
          ...
          <p className="text-muted-foreground">No blocks for {localeId} yet.</p>
          <PermissionGate permission={PERMISSIONS.CMS_EDIT}>
            ...
            return (
              <Card key={block.id}>
                <CardHeader className="pb-2">
                  ...
                  <InheritanceBadge
                    scopeType={(block as Record<string, unknown>).scope_type as "country" | "locale" | null}
                    overrideStatus={(block as Record<string, unknown>).override_status as "inherited" | "overridden" | "detached" | null}
                    inheritsFromId={(block as Record<string, unknown>).inherits_from_id as string | null}
                    module="blocks"
                    country={country}
                    lang={lang}
                    contentId={block.id}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={selectedType} onValueChange={(v) => setSelectedType(v as BlockType)} disabled={!!editItem}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {BLOCK_TYPES.map((type) => (
                      <SelectItem key={type} value={type}>{BLOCK_REGISTRY[type].label}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
